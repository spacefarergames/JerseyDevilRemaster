...
QFXRenderer :: QFXRenderer()
{
	...
	m_iTextureInit = 0;
	m_uiHudViewportLogCount = 0;
	m_uiHudVertexLogCount = 0;
	m_bHudOrthoActive = GL_FALSE;
	m_HudOrthoLeft = 0.0;
	m_HudOrthoRight = 0.0;
	m_HudOrthoBottom = 0.0;
	m_HudOrthoTop = 0.0;
	m_HudVertexScale = 1.0;
	m_HudSafeLeft = 0.0;
	m_HudSafeRight = 0.0;
	...
	m_varHudWidescreenFix = QFXSettings::Instance().GetInteger( "HudWidescreenFix", 1 );
	m_varHudViewportExpand = QFXSettings::Instance().GetFloat( "HudViewportExpand", 1.0f );
	m_varHudVertexExpand = QFXSettings::Instance().GetFloat( "HudVertexExpand", 0.0f );
	m_varHudTranslateExpand = QFXSettings::Instance().GetFloat( "HudTranslateExpand", 1.0f );
	m_varHudForceVertexExpand = QFXSettings::Instance().GetInteger( "HudForceVertexExpand", 0 );
	m_varHudViewportAspect = QFXSettings::Instance().GetFloat( "HudViewportAspect", 1.333333f );
	m_varHudViewportAspectTolerance = QFXSettings::Instance().GetFloat( "HudViewportAspectTolerance", 0.08f );
	m_varHudViewportExpand = std::min( 1.0f, std::max( 0.0f, m_varHudViewportExpand ) );
	m_varHudVertexExpand = std::min( 1.0f, std::max( 0.0f, m_varHudVertexExpand ) );
	m_varHudTranslateExpand = std::min( 1.0f, std::max( 0.0f, m_varHudTranslateExpand ) );
	m_varHudViewportAspect = std::max( 0.1f, m_varHudViewportAspect );
	m_varHudViewportAspectTolerance = std::min( 0.5f, std::max( 0.001f, m_varHudViewportAspectTolerance ) );
	...
	m_varTextureAccuracyFix = QFXSettings::Instance().GetInteger( "TextureAccuracyFix", 1 );
	m_varForcePerspectiveCorrection = QFXSettings::Instance().GetInteger( "ForcePerspectiveCorrection", 1 );
	m_varClampTextureLodBias = QFXSettings::Instance().GetInteger( "ClampTextureLodBias", 1 );
	m_varForceLinearMagnification = QFXSettings::Instance().GetInteger( "ForceLinearMagnification", 1 );
	m_varNearPlaneTextureFix = QFXSettings::Instance().GetInteger( "NearPlaneTextureFix", 1 );
	m_varTextureMinNearPlane = QFXSettings::Instance().GetFloat( "TextureMinNearPlane", 1.0f );
	m_varTextureMinNearPlane = std::max( 0.001f, m_varTextureMinNearPlane );
	m_varPSXStabilityFix = QFXSettings::Instance().GetInteger( "PSXStabilityFix", 1 );
	m_varDepthPrecisionFix = QFXSettings::Instance().GetInteger( "DepthPrecisionFix", 1 );
	m_varProjectionDiagnostics = QFXSettings::Instance().GetInteger( "ProjectionDiagnostics", 0 );
	m_varTextureMaxDepthRatio = QFXSettings::Instance().GetFloat( "TextureMaxDepthRatio", 8000.0f );
	m_varDepthRangeEpsilon = QFXSettings::Instance().GetFloat( "DepthRangeEpsilon", 0.0001f );
	m_varTextureMaxDepthRatio = std::max( 10.0f, m_varTextureMaxDepthRatio );
	m_varDepthRangeEpsilon = std::min( 0.01f, std::max( 0.0f, m_varDepthRangeEpsilon ) );
	...
	m_varPlanarShadows = QFXSettings::Instance().GetInteger( "PlanarShadows", 1 );
	m_varPlanarShadowStrength = QFXSettings::Instance().GetFloat( "PlanarShadowStrength", 0.35f );
	m_varPlanarShadowRadius = QFXSettings::Instance().GetFloat( "PlanarShadowRadius", 24.0f );
	m_varPlanarShadowOffsetX = QFXSettings::Instance().GetFloat( "PlanarShadowOffsetX", 10.0f );
	m_varPlanarShadowOffsetY = QFXSettings::Instance().GetFloat( "PlanarShadowOffsetY", 18.0f );
	m_varPlanarShadowDepthScale = QFXSettings::Instance().GetFloat( "PlanarShadowDepthScale", 0.03f );
	m_varPlanarShadowMinZ = QFXSettings::Instance().GetFloat( "PlanarShadowMinZ", 0.005f );
	m_varPlanarShadowMaxZ = QFXSettings::Instance().GetFloat( "PlanarShadowMaxZ", 0.98f );
	m_varPlanarShadowStrength = std::min( 0.75f, std::max( 0.0f, m_varPlanarShadowStrength ) );
	m_varPlanarShadowRadius = std::min( 96.0f, std::max( 1.0f, m_varPlanarShadowRadius ) );
	m_varPlanarShadowDepthScale = std::min( 0.25f, std::max( 0.001f, m_varPlanarShadowDepthScale ) );
...
void QFXRenderer :: OnMakeCurrent( HDC hdc, HGLRC hglrc )
{
	...
	if ( hdc != m_hDC || hglrc != m_hGLRC ) {
		...
		SetupTextureAccuracy( GL_TEXTURE_2D );
	}
}
...
void QFXRenderer :: SetupTextureAccuracy( GLenum target )
{
	if ( !m_varTextureAccuracyFix )
		return;

	if ( m_varForcePerspectiveCorrection )
		qglHint( GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST );

	if ( target == GL_TEXTURE_2D && m_varClampTextureLodBias )
		qglTexParameterf( target, GL_TEXTURE_LOD_BIAS, 0.0f );

	if ( target == GL_TEXTURE_2D && m_varForceLinearMagnification )
		qglTexParameteri( target, GL_TEXTURE_MAG_FILTER, GL_LINEAR );
}

GLenum QFXRenderer :: SetupPerspectiveHint( GLenum target, GLenum mode )
{
	if ( m_varTextureAccuracyFix && m_varForcePerspectiveCorrection && target == GL_PERSPECTIVE_CORRECTION_HINT )
		return GL_NICEST;

	return mode;
}

GLfloat QFXRenderer :: SetupTextureParameter( GLenum target, GLenum pname, GLfloat param )
{
	if ( m_varTextureAccuracyFix && m_varClampTextureLodBias && target == GL_TEXTURE_2D && pname == GL_TEXTURE_LOD_BIAS )
		return 0.0f;

	if ( m_varTextureAccuracyFix && m_varForceLinearMagnification && target == GL_TEXTURE_2D && pname == GL_TEXTURE_MAG_FILTER )
		return static_cast<GLfloat>( GL_LINEAR );

	return param;
}

GLint QFXRenderer :: SetupTextureParameter( GLenum target, GLenum pname, GLint param )
{
	if ( m_varTextureAccuracyFix && m_varClampTextureLodBias && target == GL_TEXTURE_2D && pname == GL_TEXTURE_LOD_BIAS )
		return 0;
...
void QFXRenderer :: SetupNearPlaneFrustum( GLdouble& left, GLdouble& right, GLdouble& bottom,
	GLdouble& top, GLdouble& zNear, GLdouble& zFar )
{
	if ( !m_varTextureAccuracyFix || !m_varPSXStabilityFix || !m_varNearPlaneTextureFix )
		return;

	if ( zNear <= 0.0 || zFar <= zNear )
		return;

	GLdouble targetNear = zNear;
	targetNear = std::max( targetNear, static_cast<GLdouble>( m_varTextureMinNearPlane ) );

	if ( m_varDepthPrecisionFix ) {
		const GLdouble ratioNear = zFar / static_cast<GLdouble>( m_varTextureMaxDepthRatio );
		targetNear = std::max( targetNear, ratioNear );
	}

	if ( targetNear <= zNear )
		return;

	const GLdouble oldNear = zNear;
	const GLdouble oldRatio = zFar / oldNear;
	const GLdouble scale = targetNear / zNear;
	left *= scale;
	right *= scale;
	bottom *= scale;
	top *= scale;
	...
	if ( m_varDebug || m_varProjectionDiagnostics ) {
		QFXLog::Instance().Printf( "%5i: PSX stability adjusted frustum near %.5f -> %.5f, far %.2f, ratio %.1f -> %.1f\n",
			m_uiFrameCounter, oldNear, zNear, zFar, oldRatio, zFar / zNear );
	}
}

bool QFXRenderer :: SetupHudViewport( GLint& x, GLint& y, GLsizei& width, GLsizei& height, bool force )
{
	if ( !m_varHudWidescreenFix || !m_varHudViewportExpand )
		return false;
	...
	RECT rc;
	if ( !GetClientRect( hCurrentWnd, &rc ) )
		return false;

	const int windowWidth = rc.right - rc.left;
	const int windowHeight = rc.bottom - rc.top;
	if ( windowWidth <= 0 || windowHeight <= 0 || windowWidth <= width )
		return false;
	...
	if ( fabsf( viewportAspect - m_varHudViewportAspect ) > m_varHudViewportAspectTolerance )
		return false;

	const int expectedCenteredX = ( windowWidth - width ) / 2;
	const int centerTolerance = std::max( 4, windowWidth / 100 );
	if ( abs( x - expectedCenteredX ) > centerTolerance && x != 0 )
		return false;

	const float amount = m_varHudViewportExpand;
	const GLint oldX = x;
	const GLint oldY = y;
	const GLsizei oldWidth = width;
	const GLsizei oldHeight = height;
	const float newX = static_cast<float>( x ) * ( 1.0f - amount );
	const float newWidth = static_cast<float>( width ) + static_cast<float>( windowWidth - width ) * amount;

	x = static_cast<GLint>( newX + 0.5f );
	width = static_cast<GLsizei>( newWidth + 0.5f );
	if ( m_varDebug && m_uiHudViewportLogCount < 16 ) {
		QFXLog::Instance().Printf( "HUD widescreen rectangle expanded: %i,%i %ix%i -> %i,%i %ix%i\n",
			oldX, oldY, oldWidth, oldHeight, x, oldY, width, height );
		++m_uiHudViewportLogCount;
	}
	return true;
}

void QFXRenderer :: SetupHudOrtho( GLdouble left, GLdouble right, GLdouble bottom, GLdouble top )
{
	m_bHudOrthoActive = GL_FALSE;
	m_HudVertexScale = 1.0;
	m_HudOrthoLeft = left;
	m_HudOrthoRight = right;
	m_HudOrthoBottom = bottom;
	m_HudOrthoTop = top;
	m_HudSafeLeft = left;
	m_HudSafeRight = right;

	if ( !m_varHudWidescreenFix || !m_varHudVertexExpand )
		return;

	const GLdouble orthoWidth = fabs( right - left );
	const GLdouble orthoHeight = fabs( top - bottom );
	if ( orthoWidth <= 0.0 || orthoHeight <= 0.0 )
		return;
	...
	const float windowAspect = static_cast<float>( windowWidth ) / static_cast<float>( windowHeight );
	const float orthoAspect = static_cast<float>( orthoWidth / orthoHeight );
	if ( windowAspect <= m_varHudViewportAspect )
		return;

	if ( !m_varHudForceVertexExpand && fabsf( orthoAspect - m_varHudViewportAspect ) > m_varHudViewportAspectTolerance )
		return;

	m_HudVertexScale = 1.0 + ( ( static_cast<GLdouble>( windowAspect ) / static_cast<GLdouble>( m_varHudViewportAspect ) ) - 1.0 ) * m_varHudVertexExpand;
	const GLdouble center = ( left + right ) * 0.5;
	const GLdouble safeWidth = orthoHeight * static_cast<GLdouble>( m_varHudViewportAspect );
	m_HudSafeLeft = center - safeWidth * 0.5;
	m_HudSafeRight = center + safeWidth * 0.5;
	m_bHudOrthoActive = GL_TRUE;
	if ( m_varDebug && m_uiHudVertexLogCount < 4 ) {
		QFXLog::Instance().Printf( "HUD ortho vertex remap enabled: ortho %.1fx%.1f aspect %.3f window aspect %.3f scale %.3f safe %.1f..%.1f\n",
			static_cast<float>( orthoWidth ), static_cast<float>( orthoHeight ), orthoAspect, windowAspect, static_cast<float>( m_HudVertexScale ),
			static_cast<float>( m_HudSafeLeft ), static_cast<float>( m_HudSafeRight ) );
		++m_uiHudVertexLogCount;
	}
}

GLdouble QFXRenderer :: SetupHudVertexX( GLdouble x )
{
	if ( !m_bHudOrthoActive || !m_varHudWidescreenFix || !m_varHudVertexExpand )
		return x;

	const GLdouble orthoWidth = fabs( m_HudOrthoRight - m_HudOrthoLeft );
	const GLdouble orthoHeight = fabs( m_HudOrthoTop - m_HudOrthoBottom );
	if ( orthoWidth <= 0.0 || orthoHeight <= 0.0 )
		return x;

	if ( m_varHudForceVertexExpand && ( x < m_HudSafeLeft || x > m_HudSafeRight ) )
		return x;
	...
	const GLdouble center = ( m_HudOrthoLeft + m_HudOrthoRight ) * 0.5;
	return center + ( x - center ) * m_HudVertexScale;
}

GLdouble QFXRenderer :: SetupHudTranslateX( GLdouble x )
{
	if ( !m_bHudOrthoActive || !m_varHudWidescreenFix || !m_varHudTranslateExpand )
		return x;
	...
	const GLdouble windowAspect = static_cast<GLdouble>( windowWidth ) / static_cast<GLdouble>( windowHeight );
	if ( windowAspect <= static_cast<GLdouble>( m_varHudViewportAspect ) )
		return x;

	if ( x < m_HudSafeLeft || x > m_HudSafeRight )
		return x;

	const GLdouble translateScale = 1.0 + ( m_HudVertexScale - 1.0 ) * m_varHudTranslateExpand;
	const GLdouble center = ( m_HudOrthoLeft + m_HudOrthoRight ) * 0.5;
	const GLdouble fixedX = center + ( x - center ) * translateScale;
	if ( m_varDebug && m_uiHudVertexLogCount < 16 && fabs( fixedX - x ) > 0.01 ) {
		QFXLog::Instance().Printf( "HUD translate remap: %.2f -> %.2f\n", static_cast<float>( x ), static_cast<float>( fixedX ) );
		++m_uiHudVertexLogCount;
	}
	return fixedX;
}
...
void QFXRenderer :: CheckHardware()
{
	...
	m_bSupportsPlanarShadows = m_varPlanarShadows ? (( ( ext.maxActiveTextures >= 2 ) && ext.VertexShader && ext.FragmentShader && ext.RectangleTexture && ext.DepthTexture ) ? GL_TRUE : GL_FALSE ) : GL_FALSE;
	...
	QFXLog::Instance().Printf( "Planar shadows: %s\n", m_varPlanarShadows ? (m_bSupportsPlanarShadows ? "YES" : "NO") : "DISABLED" );
...
void QFXRenderer :: InitializeGL()
{
	...
	if ( m_bSupportsPlanarShadows ) {
		m_pPlanarShadowShader = new QFXShader( g_szVertexShader_PlanarShadow, g_szFragmentShader_PlanarShadow );
		if ( !m_pPlanarShadowShader || !m_pPlanarShadowShader->IsValid() ) m_bSupportsPlanarShadows = GL_FALSE;
	}
...
void QFXRenderer :: InitializeTextures()
{
	...
	if ( ext.RectangleTexture ) {
		...
		if ( m_bSupportsBloom || m_bSupportsSSAO || m_bSupportsPlanarShadows || m_bSupportsDOF ) {
		...
		if ( m_bSupportsSSAO || m_bSupportsPlanarShadows || m_bSupportsDOF ) {
	...
	if ( !m_uiScreenRGB ) {
		...
		m_bSupportsPlanarShadows = GL_FALSE;
...
void QFXRenderer :: ShutdownGL()
{
	...
	if ( m_pPlanarShadowShader ) {
		delete m_pPlanarShadowShader;
		m_pPlanarShadowShader = nullptr;
	}
	if ( m_pHSLShader ) {
		delete m_pHSLShader;
...
void QFXRenderer :: SetupZDimensions( GLfloat znear, GLfloat zfar )
{
	...
	if ( m_varDebug || m_varProjectionDiagnostics ) {
		const GLfloat ratio = ( m_zNear > 0.0f ) ? ( m_zFar / m_zNear ) : 0.0f;
		QFXLog::Instance().Printf("%5i: SetupZDimensions zn = %f, zf = %f, ratio = %f\n", m_uiFrameCounter, m_zNear, m_zFar, ratio );
	}
}
...
{
	const GLclampd originalNear = zNear;
	const GLclampd originalFar = zFar;
	...
	if ( m_varPSXStabilityFix && m_varDepthPrecisionFix ) {
		zNear = std::min( 1.0, std::max( 0.0, zNear ) );
		zFar = std::min( 1.0, std::max( 0.0, zFar ) );
		if ( fabs( zFar - zNear ) < m_varDepthRangeEpsilon ) {
			if ( zFar >= zNear )
				zFar = std::min( 1.0, zNear + m_varDepthRangeEpsilon );
			else
				zNear = std::min( 1.0, zFar + m_varDepthRangeEpsilon );
		}
	}

	if ( ( m_varDebug || m_varProjectionDiagnostics ) && ( originalNear != zNear || originalFar != zFar ) ) {
		QFXLog::Instance().Printf( "%5i: PSX stability adjusted depth range %.6f..%.6f -> %.6f..%.6f\n",
			m_uiFrameCounter, originalNear, originalFar, zNear, zFar );
	}
...
void QFXRenderer :: PostProcess()
{
	...
	// render projected depth-based shadows
	if ( m_bSupportsPlanarShadows )
		PostProcess_PlanarShadows();
...
void QFXRenderer :: PostProcess_PlanarShadows()
{
	...
	m_pPlanarShadowShader->Bind();
	m_pPlanarShadowShader->SetParameter4f( 0, 2.0f * m_zNear, m_zFar - m_zNear, (float)screenWidth, (float)screenHeight );
	m_pPlanarShadowShader->SetParameter4f( 1, m_varPlanarShadowStrength, m_varPlanarShadowDepthScale, m_varPlanarShadowMinZ, m_varPlanarShadowMaxZ );
	m_pPlanarShadowShader->SetParameter4f( 2, m_varPlanarShadowOffsetX, m_varPlanarShadowOffsetY + m_varPlanarShadowRadius, 0, 0 );
	qglBindTexture( GL_TEXTURE_RECTANGLE_ARB, m_uiScreenRGB );
	qglActiveTextureARB( GL_TEXTURE1_ARB );
	qglBindTexture( GL_TEXTURE_RECTANGLE_ARB, m_uiScreenDepth );
	qglActiveTextureARB( GL_TEXTURE0_ARB );
	RenderFSQ( screenWidth, screenHeight );
	m_pPlanarShadowShader->Unbind();
}
